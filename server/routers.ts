...
import { createAccount, createEmployee, createInvoice, createJournalEntry, createParty, createProduct, createPayrollRun, createStockMove, getDashboardSummary, listAccounts, listEmployees, listInvoices, listJournalEntries, listNotifications, listParties, listProducts, saveAttachment, updateUserRole } from "./db";
...
const invoiceInput = z.object({ invoiceNumber: z.string().min(1), type: z.enum(["sale", "purchase"]), partyId: z.number().optional(), invoiceDate: z.coerce.date(), discount: z.coerce.number().min(0).default(0), tax: z.coerce.number().min(0).default(0), paid: z.coerce.number().min(0).default(0), notes: z.string().optional(), lines: z.array(z.object({ productId: z.number().optional(), description: z.string().min(1), quantity: z.coerce.number().positive(), unitPrice: z.coerce.number().min(0), taxRate: z.coerce.number().min(0).max(100).default(0) })).min(1) });
